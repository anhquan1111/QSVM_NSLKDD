...
5.  Apply SelectKBest(f_classif, k=15) — fit on train only — to eliminate
    sparse/noisy OHE columns while retaining the 15 most statistically
    significant features.  Reducing to 15 (rather than the previous 20)
    removes more inter-correlated OHE columns, giving PCA a cleaner,
    lower-noise input and improving the information density of each of the
    4 principal components that are ultimately encoded as qubit rotation
    angles in the 16-dimensional Hilbert space.
6.  Apply PCA(n_components=4) — fit on train only — on the 15 features
    selected above, to satisfy the hard 4-qubit hardware constraint of the
...
# Why 15?
# -------
# After One-Hot Encoding, the three categorical columns expand to ~85 binary
# indicator columns, most of which are extremely sparse and carry near-zero
# F-scores.  Retaining only the top-15 most statistically significant features
# (by ANOVA F-statistic) achieves two things:
#
#   1. Noise reduction — the 15 surviving columns have high linear separability
#      between Normal and Attack classes; removing low-F columns reduces the
#      "curse of dimensionality" before PCA, yielding more stable eigenvectors.
#
#   2. Hilbert-space signal quality — after PCA compresses 15 → 4 dimensions,
#      each of the 4 principal components captures a purer fraction of the
#      discriminative variance.  Those 4 values become the qubit rotation angles
#      that parameterise the ZZFeatureMap encoding:
#          U(x) = [H ⊗ P(2x_i) ⊗ ZZ-entangle]^reps
#      A higher per-component explained variance means the 16-dimensional quantum
#      state |φ(x)⟩ encodes more genuine class-separating structure, which
#      directly improves the discriminative power of the quantum kernel
#          K(x,z) = |⟨φ(x)|φ(z)⟩|²
K_BEST: int = 15
...
def apply_select_k_best(
    ...
    categorical columns alone contribute up to ~85 binary indicator columns).
    Most of these OHE columns are extremely sparse and carry little
    discriminative power.  ``SelectKBest(f_classif)`` uses the univariate
    ANOVA F-statistic to score each feature's linear separability between the
    Normal and Attack classes and retains only the top ``k=15``.

    Why k=15 improves the quantum kernel representation
    ----------------------------------------------------
    The ZZFeatureMap encodes k values (here k=4 after PCA) as rotation angles
    on the Bloch sphere.  The quality of the resulting quantum kernel
        K(x, z) = |⟨φ(x)|φ(z)⟩|²
    depends on how much genuine class-separating information is packed into
    those 4 angles.  Feeding SelectKBest(k=15) → PCA(4) instead of
    SelectKBest(k=20) → PCA(4) produces:

    - Higher per-component explained variance (less noise dilutes the
      principal components because 5 low-F columns are removed).
    - More stable PCA eigenvectors (fewer near-zero-variance OHE columns
      create ill-conditioned covariance matrices).
    - Tighter class clusters in the 4-D PCA space, which map to more
      discriminative quantum states in the 16-D Hilbert space C^16.
    ...
    k:
        Number of features to keep.  Default is ``K_BEST`` (15).
    ...
    selector : SelectKBest
        Fitted selector object, serialised as ``models/feature_selector.joblib``
        for inference reuse.
    """
...
def apply_pca(
    ...
    --------------------------
    SelectKBest retains 15 features with the highest ANOVA F-scores.  PCA
    then extracts the *directions of maximum variance* in that 15-D space and
    projects onto the 4 principal axes.  This two-stage Hybrid pipeline
    (statistical filter → linear projection) is superior to applying PCA
    directly to the ~120 OHE-expanded feature space because:

    - Fewer input columns (15 vs ~120) → more stable covariance matrix
      estimation, less influence from near-zero-variance OHE indicator columns.
    - Higher cumulative explained variance in the 4 PCs because low-F columns
      (mostly noise variance) are excluded before the eigenvector computation.
    - Better Hilbert-space representation: each of the 4 PCA values becomes a
      qubit rotation angle θ_i ∈ [0, π].  Purer principal components → more
      genuinely discriminative angles → stronger quantum kernel signal.
    ...
    """
    logger.info("Applying PCA(n_components=%d) on %d SelectKBest features …",
    ...
    # --- Per-component explained variance report ----------------------------
    # Each ratio shows what fraction of the total variance in the 15-D
    # SelectKBest space is captured by that principal component.
    # Higher per-component ratios indicate that the component encodes a
    # genuine, high-magnitude direction of class-separating variance — which
    # translates directly into a more informative qubit rotation angle.
    evr   = pca.explained_variance_ratio_
    cumev = evr.cumsum() * 100
    logger.info("  PCA Explained Variance Ratios (per component):")
    for i, (ratio, cum) in enumerate(zip(evr, cumev)):
        logger.info(
            "    PC%d : %.6f  (%6.3f%%)  |  cumulative: %7.4f%%",
            i + 1, ratio, ratio * 100, cum,
        )
    logger.info(
        "  EXACT cumulative variance retained by %d PCs: %.6f%%",
        n_components, evr.sum() * 100,
    )
    logger.info(
...
def save_artifacts(
    ...
    # --- Transformer objects ---
    model_artifacts = {
        "feature_selector.joblib": selector,      # SelectKBest(f_classif, k=15)
        "pca_4components.joblib":   pca,           # PCA(n_components=4)
        "scaler_minmax_pi.joblib":  scaler,        # MinMaxScaler([0, π])
    }
...
def run_pipeline() -> None:
    ...
    # ------------------------------------------------------------------
    # 5+6. Hybrid feature selection (SelectKBest k=15 — fit on train ONLY)
    #
    # SelectKBest(f_classif, k=15) scores every post-OHE column by its
    # ANOVA F-statistic against the binary label and keeps the top 15.
    # Reducing from the previous k=20 removes 5 additional low-information
    # OHE columns, giving PCA a cleaner 15-D input and improving the
    # information density of each principal component — and therefore each
    # qubit rotation angle — in the 16-dimensional Hilbert space.
    # ------------------------------------------------------------------
    X_train_sel, X_test_sel, selector = _apply_select_k_best_internal(
        X_train_np, y_train_np, X_test_np, k=K_BEST
    )
    logger.info(
        "  Hybrid pipeline: %d OHE features -> SelectKBest(k=%d) -> PCA(%d) -> angles",
        X_train_np.shape[1], K_BEST, N_QUBITS,
    )

    # ------------------------------------------------------------------
    # 7. PCA on the 15 SelectKBest features (fit on train ONLY)
    # ------------------------------------------------------------------

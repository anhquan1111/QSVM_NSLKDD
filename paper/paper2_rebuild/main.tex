...
Reliability is not uniform in the amount of training data. Sweeping
$N\in\{\pldNmin,\ldots,\pldNmax\}$ and refitting every model on each subset,
the best-calibrated model at the smallest size is \pldWinnerMin{}
($\pldQsvmMin$ against $\pldMlpMin$ for the MLP and $\pldRfMin$ for Random
Forest). The quantum kernel's relative standing falls as $N$ grows: at
$N=\pldNmax$ it is $\pldQsvmMax$ against $\pldMlpMax$ for the MLP. The
advantage such as it is belongs to the low-data regime, which is also the
regime in which a four-qubit kernel is cheapest to run.

\subsection{Class-prior shift}
We rebuild three operating conditions from the full test split rather than
from a fixed sample: a balanced mix ($\ppsBalN$ records), an attack-heavy
mix ($\ppsAttN$), and a DoS-only mix ($\ppsDosN$). The best-calibrated model
is \ppsBalWinner{} on the balanced mix ($\ppsBalMlp$ against $\ppsBalQsvm$
for the quantum kernel), \ppsAttWinner{} on the attack-heavy mix, and
\ppsDosWinner{} on the DoS-only mix ($\ppsDosQsvm$). The quantum kernel is
ahead of both tree ensembles under all three conditions
($\ppsBalRf$, $\ppsAttRf$, $\ppsDosRf$ for Random Forest), which is the same
ordering as Table~\ref{tab:main} and not a separate effect.

...
%
%  Doi chieu so lieu: python runners/audit_prose.py  (102/102, gom ca thu nay)
% =====================================================================
...
% Khoi trich y reviewer. Mau xam + gach doc de tach han khoi phan tra loi:
% AE doc luot phai phan biet duoc ngay dau la loi ho, dau la loi minh.
\newenvironment{revcomment}
  {\par\vspace{4pt}\noindent
   \begin{list}{}{\leftmargin=1.2em \rightmargin=0.4em}\item[]
   \color{quotegray}\itshape}
  {\end{list}\vspace{2pt}}

\newcommand{\itemhead}[2]{%
  \par\vspace{10pt}\noindent
  {\bfseries #1}\quad{\bfseries #2}\par\nobreak\vspace{-2pt}
  {\color{rulegray}\hrule height 0.4pt}}

\newcommand{\resp}{\par\vspace{3pt}\noindent\textbf{Response.}\ }
\newcommand{\changed}{\par\vspace{2pt}\noindent\textbf{Changes.}\ }
...
\changed
New Sec.~VI (Regime Map), Fig.~10, and a rewritten abstract and introduction.
...
\changed
All results sections; Figs.~7--12.
...
processor, and the manuscript now says so in those words.
\changed
...
\end{revcomment}
\resp
The work is now cited and discussed. We must report a limitation honestly: the
publisher's full text is not accessible to us, and we were unable to retrieve
the author list from either the publisher's page or secondary indexes. We have
therefore cited the article by title, venue and DOI, and we ask the editor
whether the author list can be supplied so that the citation can be completed.
We did not guess at it. Where our positioning table would require details only
the full text contains, the cells are marked \emph{n/r} rather than filled by
inference.

What we could establish from the abstract is stated in Sec.~II-B: the study
evaluates Pegasos-QSVC, VQC and a hybrid network on ToN\_IoT and NSL-KDD under
ideal and noisy IBM simulators, with Pegasos-QSVC best at $94.60\%$ accuracy. It
varies the model family and the noise model; it does not vary the embedding
width or the training-set size, and it does not include a gradient-boosted tree
baseline. That is the axis on which our contribution lies.
\changed
...
training).
\changed
Sec.~IV-A, Sec.~V-F, Sec.~VII-D, Fig.~11.
...
\changed
Sec.~IV-B; Table~II reports both arms across all sizes.
...
for about $+0.006$. The revision reports both test sets everywhere and states
which is which.
\changed
Sec.~IV-B; \texttt{c4\_table\_iv\_vs\_vi.csv} in the repository.
...
the official split throughout. This is now stated in the manuscript so that our
absolute numbers are not read as underperformance.
\changed
...
\changed
Sec.~III (F3), Sec.~V-G, Fig.~12, Sec.~VII-B.
...
\changed
Sec.~V-C, Sec.~VI, Fig.~10; \texttt{regime\_map\_rows.csv}, all $110$ rows
...
page range --- and could not locate the item either. We have removed it. The
sentence it supported has been rewritten so that it no longer requires that
citation. We record the removal in the bibliography file with the original
...
\changed
Sec.~V-G, Fig.~12, Sec.~VII-A, Assumption~1.
...
not. Bowles \emph{et al.} vary model family across $160$ datasets; Schnabel and
Roth vary kernel type across $64$; the closest concurrent work on our exact
datasets, Gillani \emph{et al.} (2026), varies circuit width. \emph{None varies
the training-set size}, and that is the variable that determines the answer
here. We do not claim breadth over these studies --- Gillani \emph{et al.}
evaluate four datasets to our two, at eight qubits to our four and six.
\changed
...
malware detection}
\resp
Cited and discussed in Sec.~II. We were unable to obtain the full text through
our institutional access and have positioned it from the abstract and
bibliographic record only; where the positioning table would need details from
the full text, the cells are marked \emph{n/r}.
\changed
Sec.~II-B, Table~I, bibliography.
...
Theorem~1), erratum subsection, Fig.~5 replacing the Pareto frontier figure.
...
Reviewers and the Associate Editor commonly receive a version with changes
marked. We would ordinarily supply one, but in this case a marked version would
be misleading rather than helpful: essentially every paragraph of the manuscript
is new text. The abstract, introduction, related work, framework, experimental
setup, results, limitations and conclusion have all been rewritten; the theory
section has had one theorem, one definition, one algorithm and one equation
removed and a new definition, lemma, remark and appendix added; and all twelve
figures are regenerated from re-run experiments. Marking everything would convey
no information.

We have therefore prepared this letter to serve that purpose instead: every
substantive change is listed above under the comment that motivated it, with the
section it lands in. Should the editors prefer a conventionally marked file, we
will produce one on request.

Two things did \emph{not} change, and we note them so their absence from the
above is not read as an oversight: the title, and the author list and their
affiliations.
...
Addressed with new results    & 6 &  8 & 6 & 4 & 5 & 29 \\
Addressed in part, stated as such & 0 & 2 & 0 & 1 & 1 & 4 \\
\bottomrule
\end{tabular}
\end{center}

\vspace{4pt}
The four items we describe as addressed only in part, and what remains
outstanding in each:

\begin{itemize}[leftmargin=1.3em,itemsep=2pt,topsep=3pt]
  \item \textbf{R1-1} and \textbf{R4-1} --- both works are cited and positioned,
    but a publisher paywall prevented us from reading either full text. For
    R1-1 we were additionally unable to retrieve the author list, and did not
    guess at it; we would welcome the editor's help in completing that entry.

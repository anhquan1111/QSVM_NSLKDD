...
text through our institutional access, so the comparison rests on the abstract
and the bibliographic record, and we say so in the text and present the parallel
as suggestive rather than as evidence. For the same reason the study is
discussed in prose but not tabulated in Table~I, whose cells are otherwise read
from full texts.
\changed

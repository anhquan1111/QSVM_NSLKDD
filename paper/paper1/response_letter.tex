...
released repository. Five audit scripts are provided, and we invite the
...
\texttt{verify\_noise10.py} & 40/40 &
  The ten-run noise check: per-condition means, paired intervals, Wilcoxon
  $p$-values and the Holm thresholds, from the per-run caches \\
\bottomrule
\end{tabular}
\end{center}
...
two-qubit gates at $n=4$, $r=2$. The check runs on all ten training runs at
$512$ shots per kernel entry, so it carries a paired confidence interval rather
than a point estimate. The entanglement conclusion is unchanged under noise.
\changed
Sec.~V-B, Sec.~VII-A.
...
all figure-generation scripts, and the five audit scripts listed above. The link
...
A marked-up version accompanies this submission. Because essentially every
paragraph of the manuscript is new text --- the abstract, introduction, related
work, framework, experimental setup, results, limitations and conclusion have
all been rewritten, the theory section has had one theorem, one definition, one
algorithm and one equation removed and a new definition, lemma, remark and
appendix added, and all figures are regenerated from re-run experiments ---
marking individual sentences would convey no information. We have instead
tagged the manuscript at \emph{subsection} granularity. Each subsection heading
in the marked copy carries one of four tags --- \textsc{new}, \textsc{rewritten},
\textsc{corrects an error}, \textsc{retained} --- together with one sentence
stating what changed and the item in this letter that prompted it.

The marked copy also lists, on its cover page, the material \emph{removed} from
the submitted version, which by its nature cannot be tagged anywhere in the
body: Theorem~1, Definition~4, Algorithm~1, the objective $J(n)$, three tables
and four figures.

Both files are generated from a single source and differ in one flag, so the
marked copy and the clean copy cannot drift apart.
...
% =====================================================================
\section{Changes to the bibliography}

TETC requires bibliography changes to be itemised. There are eleven, in four
groups.

\emph{Six entries added}, four because reviewers named them and two because we
found them ourselves while checking the literature. Cirillo, Esposito and Seo,
\emph{Quantum Machine Intelligence}, 2026, named at R1-1. Bowles, Ahmed and
Schuld, arXiv:2403.07059, and Schnabel and Roth, arXiv:2409.04406, both named at
R3-5. Carducci, ICAD 2026, named at R4-1. Kaissar, Nassif and Bouridane,
\emph{Future Internet}, 2026, which replaces the withdrawn entry below. And
Gillani \emph{et al.}, arXiv:2608.18155, a concurrent study covering both of our
datasets that no reviewer raised and that we cite because omitting it would be a
material omission.

\emph{One entry removed as unlocatable.} Former reference~[26] could not be
found by two independent searches; see R2-6. The released bibliography file
records the removal together with the original string, so the change is
auditable rather than silent.

\emph{One entry corrected.} Former reference~[15] carried a wrong DOI; see R2-5.

\emph{Three entries removed as no longer relevant.} The submitted version's
contribution C5 was a confidence-calibration and rare-attack study reporting
Expected Calibration Error and AUC-PR. That contribution is not in this
revision, and with it go the three references that supported it alone: Platt
(1999), Naeini \emph{et al.} (2015) and Guo \emph{et al.} (2017), all on
probability calibration. We record the removal here rather than leave three
uncited entries standing in the list.

After these changes the bibliography holds $38$ entries, every one of them cited
at least once in the manuscript text, against the journal's limit of $45$. No
self-citation was added or removed.

...
\end{revcomment}
\resp
The work is now cited and discussed: Cirillo, Esposito and Seo,
\emph{Quantum Machine Intelligence}, vol.~8, no.~1, art.~27, 2026.

We should report one limitation honestly. The publisher's full text is not
accessible to us, so our discussion rests on the abstract and the bibliographic
record; where our positioning table would require details only the full text
contains, the cells are marked \emph{n/r} rather than filled by inference.

What we can state is in Sec.~II-B: the study evaluates Pegasos-QSVC, a
variational classifier and a hybrid quantum--classical network on ToN\_IoT and
NSL-KDD under ideal and noisy IBM simulators, with Pegasos-QSVC best at
$94.60\%$ accuracy. It varies the model family and the noise model; it does not
vary the embedding width or the training-set size, and it does not include a
gradient-boosted tree baseline. That is the axis on which our contribution lies,
and we would welcome the reviewer's correction if the full text contradicts this
reading.
\changed
...
\begin{itemize}[leftmargin=1.3em,itemsep=2pt,topsep=3pt]
  \item \textbf{R1-1} and \textbf{R4-1} --- both works are cited with complete
    bibliographic records and positioned in Table~I, but a publisher paywall
    prevented us from reading either full text, so a small number of cells in
    that table are marked \emph{n/r} rather than filled by inference.

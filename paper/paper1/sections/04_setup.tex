...
\revnote{rewrite}{Them UNSW-NB15 thanh dataset chinh (y AE-3, R1-2) va so lieu ve trung lap cua ca hai bo.}
...
\subsection{Protocol}
\revnote{fix}{Ban da nop: 5 seed, va QSVM co dinh C=1.0 trong khi SVM co dien duoc tune -- Reviewer 1 chi ra o y R1-3 rang cach nay thien vi chinh phuong phap cua ta. Nay: 10 run, tune doi xung, hai nhanh, hai tap test.}
...
\revnote{rewrite}{Them hieu chinh Holm trong tung ho baseline (y R2-3, R2-4).}
...
\revnote{new}{Them link repo, commit hash va bon bo kiem tu dong -- Reviewer 4 chi ra o y R4-3 rang ban cu noi reproducible ma khong dua gi.}
...
Qiskit reference to within $1.3\times10^{-15}$ and which makes the sweeps in
...
every intermediate artifact, all figure-generation scripts, and five audit
scripts are released at
\url{https://github.com/anhquan1111/QSVM_NSLKDD}, at commit
\texttt{19beb18}.\footnote{Commit hash to be updated to the final revision
...
\texttt{audit\_prose.py} (132) re-derives every number written in the text of
this paper. \texttt{verify\_lemma1.py} (15) checks the expansion of
Lemma~\ref{lem:expansion} against the exact kernel. \texttt{verify\_noise10.py} (40)
re-derives the ten-run noise check of Sec.~\ref{sec:res-c2}, including the
paired intervals and the Holm thresholds, from the per-run caches. Together
they found four errors in our own revision code before publication, one of
which changed a reported $n^{\ast}$.

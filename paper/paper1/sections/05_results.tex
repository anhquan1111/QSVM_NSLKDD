...
\label{sec:res-c1}
\revnote{rewrite}{Bo Fig. 5 (Pareto frontier) cua ban cu; thay bang hinh quet K va hinh luat ba giai doan tren CA HAI dataset.}
...
\label{sec:res-c2}
\revnote{fix}{So lieu QSVM GIU NGUYEN. Nhung them Random Forest va XGBoost (y AE-4, R1-5, R2-1) thi THU HANG DOI: XGBoost 0.8503 vuot QSVM-ZZ 0.8469. Ban da nop ghi QSVM dung dau.}
...
Under the backend-derived noise model the conclusion is unchanged, and we can
now state it over ten runs rather than one. Repeating the check on all ten
training runs at $512$ shots per kernel entry, \QSVM{} attains a mean
macro-$F_1$ of $0.8329$ under exact statevector simulation, $0.8363$ with finite
shots and $0.8501$ under \texttt{FakeManilaV2} noise, against $0.8309$,
$0.8323$ and $0.8307$ for QSVM-\Zmap{}. Paired against exact simulation,
\QSVM{} gains $+0.0172$ $[+0.0021,+0.0324]$ under noise ($p=0.027$, $d_z=0.81$)
while QSVM-\Zmap{} is flat at $-0.0002$ $[-0.0115,+0.0111]$. We report the
first as inconclusive rather than as an improvement, for the reason the rest of
this paper applies: the smallest $p$ this check produces is $0.027$, which
clears no first Holm threshold for any family we could defend --- from the two
noise-versus-ideal comparisons ($0.05/2$) to all nine the check entails
($0.05/9$). Noise at this depth does not measurably degrade macro-$F_1$, and now
there is an interval behind that statement instead of a single point.

The Gram matrix moves far more than the score does. The relative Frobenius
distance between the noisy and the exact Gram matrix averages $0.602$ for
\QSVM{} against $0.165$ for QSVM-\Zmap{}, and alignment falls from $0.194$
to $0.149$ against $0.070$ to $0.068$: noise disturbs the entangling map's
geometry several times more than the control's, and neither disturbance reaches
macro-$F_1$. That is the same disconnect between kernel geometry and end-task
score reported for the ablation itself above.

\subsection{Distribution shift}
\label{sec:res-c3}
\revnote{fix}{Them RF/XGBoost thi KET LUAN DOI CHIEU: QSVM tut nhanh hon ca hai ensemble cay. Ban da nop goi che do nay la bang chung manh nhat cua loi the.}
...
\label{sec:res-c4}
\revnote{fix}{KET QUA DAO CHIEU. Ban da nop quet N toi 1000 va ket luan co loi the o che do it du lieu. Quet toi $N=10^4$ o ti le lop tu nhien thi nguoc lai: co dien thang o N nho. Tra loi truc tiep y R1-7.}

\input{tables/crossover_arms}

\input{figs_revision/fig_learning_curve}
...
\label{sec:res-rare}
\revnote{new}{MUC MOI. Ban da nop khang dinh ``+6.7 diem rare, d=+0.68'' nhung code cu khong he tinh metric nao cho lop hiem -- Reviewer 4 hoi thang o y R4-5. Khang dinh do da rut.}
...
\label{sec:res-unsw}
\revnote{new}{MUC MOI. Ban da nop chi nhac UNSW mot cau trong phan han che; Reviewer 1 va AE doi dataset thu hai lam that (y AE-3, R1-2).}
...
\label{sec:res-width}
\revnote{new}{MUC MOI. Tra loi y R3-4 (``toan mo phong o so qubit rat nho''): mo rong len 8 qubit thi 48/48 so sanh nghieng ve co dien, kem co che do duoc tu ma tran Gram.}

\input{figs_revision/fig_width}

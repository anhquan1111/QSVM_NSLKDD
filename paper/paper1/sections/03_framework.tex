...
\revnote{rewrite}{The submitted version had only Eq. (5); Eqs. (1)--(4) are now written out.}
\begin{revbody}
...
\begin{equation}
\label{eq:phase}
  U_{\Phi}(\bm{x}) = \exp\Bigl(
    \mathrm{i}\sum_{i=1}^{n} x_i Z_i
    + \mathrm{i}\!\!\sum_{1\le i<j\le n}\!\!
      \tfrac{1}{2}\varphi_{ij}(\bm{x})\,Z_iZ_j
  \Bigr),
\end{equation}
with the pairwise phase $\varphi_{ij}(\bm{x}) = 2(\pi-x_i)(\pi-x_j)$.
...
\revnote{rewrite}{Problem 1 deliberately weakened: a negative answer is admitted as a result (AE-1, R1-4).}
\begin{revbody}
...
\subsection{Pipeline}
\revnote{rewrite}{The submitted version had Fig. 2, Fig. 3 and Table II here; all three removed and replaced by prose. A statement that every baseline consumes the same four-component representation is added.}
\begin{revbody}

The path from raw records to a decision is: one-hot encoding to $d=122$
dimensions; an ANOVA $F$-test retaining $K$ features; PCA to $n$ components; a
min--max scaler mapping each component into $[0,\pi]$, the domain on which the
feature map's phases are defined; the \ZZ{}FeatureMap of
Eq.~\eqref{eq:featuremap} on the resulting $\bm{z}\in[0,\pi]^{n}$; and a
soft-margin SVC on the precomputed Gram matrix.

Every classical baseline in this paper, the three kernel SVMs, the random
forest and XGBoost, consumes the same $n$-component vector $\bm{z}$ that the
circuit encodes, not the $K$ selected features and not the $d=122$ one-hot
columns. The comparison therefore isolates the kernel from the front-end: it
asks whether a quantum kernel on a given four-dimensional embedding beats a
classical learner on that same embedding. It does not ask whether the pipeline
beats a tree ensemble on the raw record; Sec.~\ref{sec:res-c4} reports that
comparison separately, as a reference arm outside the paired protocol.

Two properties of this pipeline carry the experimental design.

\emph{One component varies at a time.} The entanglement-free control differs
from the main path in exactly one block: the \Zmap{}FeatureMap applies the same
Hadamard and single-qubit phase layers and omits the pairwise term of
Eq.~\eqref{eq:phase}. It has the same qubit count, the same repetitions, the
same classical front-end, and the same classifier, so a difference between the
two branches is attributable to entanglement and to nothing else. The same
discipline governs the other three contributions: C1 fixes the representation
before any downstream metric is inspected, C3 varies only the test
distribution, and C4 varies only $N$.
...
\revnote{keep}{Retained from the submitted version.}

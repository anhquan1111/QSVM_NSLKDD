...
$K^{\mathrm{Q}}_{n}$ denote the fidelity kernel induced by an $n$-qubit feature
...
Eq.~(5), $Q_{\mathrm{raw}}(n) = 10n^{2}-8n$, normalised by its value at the
largest candidate so that $Q(n)\in(0,1]$. The quantity it tracks is the
...
|\langle\phi(\bm{x}^{\prime})|\phi(\bm{x})\rangle|^{2}$
is evaluated exactly rather than by sampling. Because the \ZZ{}FeatureMap is

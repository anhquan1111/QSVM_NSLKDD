...
paying off once $n$ grows.

We could not obtain the full text of \cite{carducci2026} through our
institutional access. This comparison therefore rests on its abstract,
introduction and index terms, which report a retrieval setting evaluated by
Recall@$k$ rather than a classification benchmark, and which list kernel methods
among the techniques involved; we could not establish its qubit count or whether
it varies the size of its training data. We present the parallel as suggestive
rather than as evidence, and we do not tabulate the study in

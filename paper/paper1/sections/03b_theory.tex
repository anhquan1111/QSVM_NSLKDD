...
\revnote{rewrite}{Propositions 1, 3 and 4 with proofs demoted to cited facts (F1)--(F3), as R3-3 and R4-4 ask.}
\begin{revbody}
...
\revnote{fix}{Replaced entirely. The submitted version used Eq. (6) J(n), Definition 4 (Pareto), Algorithm 1 and Theorem 1; Reviewer 4 showed Theorem 1 false and the Pareto step vacuous (R4-2). Replaced by a lexicographic three-stage rule with no weights.}
\input{figs_revision/fig_c1_selection}
\begin{revbody}
...
\revnote{fix}{The submitted version used Proposition 4 and a two-sample z-test that treated the two branches as independent although they share the training set. Replaced by a within-run paired protocol with Holm correction.}
\begin{revbody}
...
\label{sec:erratum}
\revnote{new}{New subsection: erratum listing four errors of the submitted version. Two were found by Reviewer 4; the other two by the authors and raised by no reviewer.}
\begin{revbody}

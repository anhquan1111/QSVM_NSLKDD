...
\revnote{rewrite}{Ban da nop chi co sai so lay mau huu han. Nay them mo hinh nhieu dung tu calibration may that (y AE-6, R1-6), va noi ro cai gi KHONG do duoc.}
...
The check covers all ten training runs at $512$ shots per kernel entry, so
Sec.~\ref{sec:res-c2} reports a paired interval and not a single point.
Under this model the entanglement conclusion is unchanged: the alignment gap
between QSVM-ZZ and QSVM-Z survives, and the macro-$F_1$ gap, although it
widens under noise to $+0.0194$ $[+0.0026,+0.0363]$, does not clear Holm
correction and we do not claim it.
...
\revnote{fix}{Ban da nop de day la ``cau hoi mo''. Nay DA DO, va ket qua bat loi cho viec mo rong mach.}
...
\revnote{rewrite}{Da them RF+XGBoost. Noi thang la KHONG them CatBoost/TabNet/FT-Transformer, kem ly do (y R1-5).}
...
\revnote{rewrite}{Da them UNSW-NB15. Them so lieu ve trung lap cua ca hai bo.}
